  ...
  <div v-else-if="media" class="min-h-screen">
    ...
    <div class="relative">
      ...
      <div class="w-full h-[50vh] md:h-[70vh] relative">
        ...
        <div v-if="media.backdrop_path" class="absolute inset-0 bg-cover bg-center"
          :style="`background-image: url(https://image.tmdb.org/t/p/original${media.backdrop_path})`"></div>
      ...
      <div class="container mx-auto px-4 absolute bottom-0 left-0 right-0 z-20 pb-8">
        <div class="flex flex-col md:flex-row items-start md:items-end gap-6">
          ...
          <div class="w-40 md:w-64 flex-shrink-0 -mb-16 md:mb-0 rounded-lg overflow-hidden shadow-2xl">
            <img v-if="media.poster_path" :src="`https://image.tmdb.org/t/p/w500${media.poster_path}`" :alt="mediaTitle"
              class="w-full h-auto" />
          ...
          <div class="flex-grow">
            ...
            <div class="mt-2 text-gray-300 flex flex-wrap items-center gap-x-4 gap-y-1">
              ...
              <div class="flex flex-wrap gap-2 mt-1 md:mt-0">
                <span v-for="genre in media.genres" :key="genre.id" class="bg-neutral-700 px-2 py-1 rounded-md text-xs">
                  {{ genre.name }}
                </span>
              </div>
            </div>
    ...
    <div class="container mx-auto px-4 pt-20 md:pt-12 pb-16">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="md:col-span-2">
          ...
          <section
            v-if="mediaStore.currentProviders.data && (mediaStore.currentProviders.data.flatrate || mediaStore.currentProviders.data.buy || mediaStore.currentProviders.data.rent)"
            class="mb-8 -mt-12">
            <div class="flex flex-wrap gap-3">
              <!-- Streaming (flatrate) -->
              <template v-if="mediaStore.currentProviders.data.flatrate">
                <img v-for="prov in mediaStore.currentProviders.data.flatrate" :key="'flatrate-' + prov.provider_id"
                  :src="`https://image.tmdb.org/t/p/original${prov.logo_path}`" :alt="prov.provider_name"
                  :title="prov.provider_name" class="h-16 w-16 object-contain rounded bg-neutral-700 p-1" />
              </template>
            </div>
          </section>
          ...
          <section v-if="media.videos && media.videos.results.length > 0" class="mb-12">
            ...
            <div class="aspect-video">
              <iframe :src="trailerUrl" frameborder="0" allowfullscreen class="w-full h-full"></iframe>
            </div>
          </section>
          ...
          <section v-if="cast && cast.length > 0" class="mb-12">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              <div v-for="person in cast.slice(0, 8)" :key="person.id"
                class="bg-neutral-800 rounded-lg overflow-hidden">
                <div class="aspect-[2/3] relative">
                  <img v-if="person.profile_path" :src="`https://image.tmdb.org/t/p/w185${person.profile_path}`"
        ...
        <div>
          ...
          <section v-if="similarMedia && similarMedia.length > 0">
            ...
            <div class="space-y-4">
              <div v-for="item in similarMedia.slice(0, 4)" :key="item.id"
                ...
                <div class="w-20 flex-shrink-0">
                  <img v-if="item.poster_path" :src="`https://image.tmdb.org/t/p/w92${item.poster_path}`"
                    :alt="type === 'movie' ? item.title : item.name" class="w-full h-full object-cover" />
                  <div v-else class="w-full h-full aspect-[2/3] bg-neutral-700 flex items-center justify-center">
                    <span class="text-gray-400 text-xs">Não disponível</span>
                  </div>
                </div>
                <div class="p-3 flex-grow">
                  ...
                  <router-link :to="{ name: 'MediaDetails', params: { type, id: item.id } }"
            ...
            <div class="mt-4">
              <router-link :to="{ name: type === 'movie' ? 'Movies' : 'TVShows' }"
...
const loadMediaDetails = async () => {
  if (type.value && id.value) {
    ...
    if (type.value === 'movie') {
      await mediaStore.getMovieProviders(id.value);
    } else {
      await mediaStore.getTVProviders(id.value);
    }
